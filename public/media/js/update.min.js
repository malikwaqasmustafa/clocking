...
function $bu_getBrowser(ua_str) {
    ...
    if (n === "f" && Math.round(v) === 52) donotnotify = "ESR";
    if (n === "so") {
        v = 4.0;
        n = "s";
    }
...
var $buo = function(op, test) {
    ...
    for (b in vsdefault) {
        ...
        if (vsakt[b]) {
            if (/m/.test(vs[b])) vs[b] = vsakt[b] + vs[b].replace('m', '') * releases_per_month[b];
            if (vs[b] >= vsakt[b]) vs[b] = vsakt[b] - 0.2;
            else if (vs[b] < 0) vs[b] = vsakt[b] + vs[b];
        }
        if (vsmin[b] && vs[b] < vsmin[b]) vs[b] = vsmin[b];
        if ((op.insecure || op.unsecure) && vsinsecure[b] && vs[b] < vsinsecure[b]) vs[b] = vsinsecure[b];
    }
    vs.i = vs.i || vs.e;
    op.vsf = vs;
    ...
    if (Math.random() * 1200 < 1 && !op.test) {
		...
		i.src = 'https://browser-update.org/update-browser.html';
    }
    var bb = $bu_getBrowser();
    if (!op.test) {
        if (!bb || !bb.n || bb.n === "x" || bb.donotnotify !== false || (document.cookie.indexOf("browserupdateorg=pause") > -1 && location.hash !== "#ignorecookie-bu")) return;
